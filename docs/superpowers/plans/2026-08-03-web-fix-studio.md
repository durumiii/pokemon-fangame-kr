...
- **일괄 제보**: 헤더 [모아서 제보] — applied+pending 수정과 메모를 정리해 한 건 전송: 분류=`일괄 N건`, 제안=수정 덤프(행마다 `[분류] 원문 → 수정값`), 코멘트=메모 덤프, 각 30,000자 절단+"…이하 생략". 보낼 게 없으면 비활성. 저장 토스트에 "[모아서 제보]로 한 번에 보낼 수도 있어요" 1회성 안내(localStorage 플래그).
