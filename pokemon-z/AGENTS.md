...
- **유지자가 판정할 자리는 재료를 갖춰 내놓는다** — 원문 · 앞뒤 대사(페이지 전문) ·
  ...
  제안 문안은 `--proposals`로 얹고, **보는 자리는 검수 스튜디오다** —
  `--review <폴더> --serve`가 그 꼴로 내고 곧바로 띄운다(`review_gui.py`, 문맥 전문·
  겪는 순서·선별 사유·판정 저장이 거기 다 있다. 이미 승인된 이벤트는 `--no-skip`이라야
  다시 뜬다). **화면을 새로 만들지 마라** — 검수 화면이 둘이면 판정이 어디 쌓였는지부터
  갈린다. 아티팩트로 올리는 것도 같은 이유로 하지 않는다.
  **문안이 아닌 판정(규칙·표기·수술)은 `--brief`로 얹는다** — 건마다 「정해 달라는 것 ·
  갈림 · 추천」이 화면 맨 위에 서고 승인·기각·보류가 `bucket:<id>`로 판정 기록에 쌓인다.
  그리고
