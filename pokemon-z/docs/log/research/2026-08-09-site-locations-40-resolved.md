...
- `Barracks` — **총사 병영**(Cuartel Mosquetero = Musketeer Barracks, 자구 일치)으로
  보이나, 보상 문맥(3rd Delinquent — 배루키)의 대사를 정본에서 못 찾아 미대조.
- `Fluxus 호수` — 향전시티의 호수(가이오가 자리). 별도 맵명이 없어 「향전시티 호수」
  같은 서술 표기가 후보.
