...
- `Fluxus 호수` = **호수 밑바닥**(`Fondo del Lago`, 맵 323) — 실측(서브 조사,
  부모 재검증). 향전시티(맵 297)에서 워프로 직접 들어가는 서브맵이고, 가이오가
  95레벨 야생전투 이벤트가 이 맵에만 박혀 있다(`grep -rla KYOGRE Data/Map*.rxdata`
  → Map323 하나). 같은 이름의 맵이 셋(323·324·337)이라 워프 그래프로 가렸다 —
  향전시티와 직결된 것은 323뿐. 위치표 표기는 **「향전호수 바닥」**(유지자 판정 2026-08-09 — 맵명 「호수 밑바닥」은 셋이나 있어 소속을 밝힌 서술 표기).
