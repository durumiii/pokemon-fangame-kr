...
**무엇을 봤나** — 저장소 실물(감사 시점 HEAD `fe63593`까지 커밋 453개 · `AGENTS.md` ·
지침 여덟 · 대장 셋 · `ROADMAP`과 티켓 열둘 · `translate/`·`tools/`·`share/`·`runa/`
스크립트 전수)과, 세션
...
**무엇을 못 봤나** — 축약본 아홉을 합치면 89,000행이라 전수로 읽지 못했다. 직접 읽은
것은 `df44e667`의 온보딩 구간과 아홉 개 전체를 대상으로 한 문자열 조회뿐이고, 나머지는
표제 조회로 훑었다. 그래서 아래 갈래는 **축약본이 아니라 저장소 실물에서 세웠다** —
커밋 453개의 메시지와 본문, 지침에 박혀 있는 사고 기록, 스크립트 실물이 근거다. 그리고
서브에이전트가 부모에게 보낸 보고는 축약본에 아예 남지 않으므로, 축약본을 다 읽었더라도
「실제로 이행됐나」는 어차피 `git log`와 파일로 다시 확인해야 했다. 08-02 이전(다른
저장소에서 돌던 조사)과 `webapp/` 하위 세션은 사정권 밖이다.

**확정도 요약** — 아래 「지침에 뚫린 구멍」의 여섯 항목과 「감사 도중에 닫힌 구멍」은
전부 **실측**이다. 항목마다 확인한 명령이나 파일의 줄을 적었고, 문서를 쓰는 동안에도
저장소가 움직였으므로 HEAD `fe63593` 시점에서 여섯 전부를 다시 확인했다. 갈래별 「그때
무엇에 걸렸나」도 커밋 메시지와 지침 본문이 근거라 **실측**이되, 그 사고가 세션 안에서
어떤 순서로 벌어졌는지까지는 확인하지 않았다.
...
세션 첫 수가 실패하고 있었다. 축약본 `df44e667`(2026-08-07 22:52) 18행이
`Read(AGENTS.md)` → `✗ File does not exist. Note: your current working directory is
/home/durumii/workspace/claude-native/pokemon-fangame-kr`이고, 이어서 39행과 496행에서
`cd pokemon-z`가 `no such file or directory`로 튕긴다(이번엔 이미 그 안에 있어서다).
어느 층에 서 있는지가 계속 흔들렸다. 저장소 뿌리는 `pokemon-fangame-kr`인데 게이트는
`pokemon-z/AGENTS.md`에 있어서, 뿌리에서 시작한 세션은 게이트를 못 찾고 손으로
더듬어 들어가야 했다. 이 감사가 도는 동안 저장소 뿌리에 게이트가 섰다
(`bf13608 docs: 저장소 뿌리에 게이트를 세운다 — 매 세션 첫 수가 실패하고 있었다`) —
이제 뿌리의 `AGENTS.md`가 게임 폴더와 `cd pokemon-z`를 가리킨다. 아래 목록에서는 뺐다.
...
곁가지로 하나 더 적어 둔다. `docs/ledger/voices.md:14`가 말투표 초안의 근거로 「스크래치패드
`voices-samples.md`, 인물당 표본 대사 24줄」을 대는데 그 파일은 이 기계에 없다(저장소에도
없고 삭제 이력에도 안 잡힌다). 근거가 스크래치패드에 있으면 대장에 남는 것은 결론뿐이라,
「사용자 판정」이 안 붙은 항목을 뒤집으려는 사람이 표본을 다시 뽑는 것 말고는 길이 없다.
대장에 근거를 걸 때는 살아남는 자리(기록층)에 두는 편이 낫다.
