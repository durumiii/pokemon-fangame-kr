...
> **⚠ 정정 (2026-08-02 밤, 포터블 루비 실측)**: 아래 원 서술은 틀렸다. 루비의
> `^`/`$`는 **줄 앵커**라 `gsub(/\s+$/,"")`가 `\n` 앞의 `\r`을 줄 끝 공백으로 먹는다.
> 따라서 **`"a\r\nb"` → `"a\nb"`이지 `"a b"`가 아니다**(`\s{2,}`까지 안 간다).
> 이 오독이 build.py 정규화와 「도달 불가 키 73개 수선」에 그대로 들어가 잘못된
> 공백형 키를 만들었고, 같은 날 밤 루비 오라클(실전 키 20,715개 전량 일치 검증)로
> 참모양(`\n`형)으로 재수선했다. 원 서술은 기록으로 남긴다:
...
**(해소, 2026-08-02 심야)** 그 「모드」가 `share/patch_intl.py`(멱등 소스 수술)로 생겨
6곳을 템플릿형으로 수술했다. 주머니 번호 보간 5곳(098·223·107)은 런타임 키
(bagPocket1~8)가 이미 번역돼 있어 비대상, 166은 그림 경로라 비대상.
