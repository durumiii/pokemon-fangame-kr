...
**repo에 남는 모드는 일곱이다.** `UI Text KR`(스크립트에 박힌 화면 문자열은 korean.dat로
...
(`uv run share/qa-edits.py`, 설치본은 안 건드린다). 잡는 것 둘 — 옛 자구가 새 자구의
부분 문자열이면 멱등이 깨져 돌릴 때마다 또 얹히고(2026-08-17 실사고), 옛/새 자구가 둘 다
없으면 patch_intl이 그 자리에서 멈춘다.
