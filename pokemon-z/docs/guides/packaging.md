...
- **모드 .rb는 1.8.7과 3.x의 공통 부분집합으로.** PC는 1.8.7이라 신형 문법(`key:`, `&.`,
  ...
  (아래 「신형 루비 실행기 호환」).
- **주입 섹션이 실리는 차례는 설치한 차례다** — 이름순이 아니다(modkit의 `declare.arrange`가
  `order` 선언만 보고 나머지는 안정 정렬로 지금 자리를 지킨다. 2026-08-21 실측: 같은 모드
  여섯을 역순으로 얹으면 주입 블록의 차례도 그대로 뒤집힌다). 그러니 **다른 모드의 상수·모듈은
  로드 시점에 없을 수 있다.** 참조는 씬 진입 훅으로 미뤄라(로드 시점 `defined?` 분기는 조용히
  무동작이 된다). 차례를 못 박아야 하면 카드의 `order`로 선언한다.
...
모드」 58표본: 초성 뽑기 · 메뉴 열쇠 보존 · 토글 양방향과 `$DEBUG` 원복 · 필터 취소 뒤
목록 정합 · 축소 지도 helper 재사용과 놓기)와
