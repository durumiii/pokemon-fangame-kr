...
- **`mod.json`의 `expects`**(섹션 제목 → 원문 md5)는 **순정 기준으로 뜬다.** 섹션 전체를
  ...
  ⚠ **감싸기 훅 모드는 기준선으로 못 푼다** — 원본을 alias로 남겨 되부르는 모드(UI KR의
  `setText`류)는 코어가 그 메서드 본문을 고치면 재대조가 「달라졌다」고 하지만 실제로는
  패치판 본문이 그대로 불린다(헛경고). 그런 모드만 `make_package.py`의 `FORCE_INJECT`에
  사유 주석과 함께 남긴다. modkit이 감싸기를 알아보게 하는 것은 그쪽 저장소 티켓감.
