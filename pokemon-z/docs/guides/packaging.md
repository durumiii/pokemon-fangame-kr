...
- **게임에서 온 문자열에 모드의 한글 문구를 그냥 잇지 마라 — 신형 루비에서 터진다.**
  주입된 모드 소스는 `ASCII-8BIT`로 실리고 `korean.dat`에서 온 값은 `UTF-8`이라, 둘 다
  한글이면 `+` 한 번에 `Encoding::CompatibilityError`다(2026-08-21 mkxp-z 3.1.3 실측).
  붙일 쪽을 `force_encoding(상대.encoding)`으로 맞춰 놓고 이어라 — 구판 루비에는
  인코딩이 없어 그 줄이 그대로 지나간다(본보기: `디버그 모드/003_DebugLists.rb`의
  `same_enc`). 형식 문자열이 아스키뿐이면 `sprintf`는 안 터진다.
