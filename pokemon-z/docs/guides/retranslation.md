...
- **모델 층 `uv run translate/screen_llm.py <out-dir>`** — 원문 대조가 필요한
  지어냄·누락·비문·오역을 오류 자리 지적(GEMBA-MQM 꼴)으로 짚는다. flash ·
  effort=low · 행당 약 $0.002. 산출 `<out-dir>/screen-llm.jsonl`.

성적(사람 정답지 실측): train(2차 133행)에서 두 층 합집합이 기계로 잡아야 할 하드
오류 9/9 · 선별률 14%, valid(3차 107행)에서 하드 1/1 · 선별률 4%. **밋밋한 재작성
같은 취향 층은 안 걸린다**(train 양성 40 중 26이 그 층) — 그건 승인 줄 보호·실기·
제보 몫이다. 선별기의 힘은 원장 두께에 비례한다 — 새 이름이 나오면 원장부터.
