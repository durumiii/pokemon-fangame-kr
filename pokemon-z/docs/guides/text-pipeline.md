...
**전역 폴백**이라, 어느 맵에서
조회될지 모르는 대사는 여기 얹으면 잡힌다(맵 절 추가분 `translate/ko/00-maps.add.jsonl` —
`build.py`가 줄마다 `map`을 보고 그 맵 꼬리에 붙인다). 맵별 등재가 먼저 이기므로 맵0에
얹어도 기존 번역을 덮지 않는다.
...
(커밋하거나 harvest로 회수한 뒤 저장). ⚠ **이 전환이 병합되면 떠 있는 스튜디오는
재시작해야 새 경로를 탄다.**
...
실제로 그럴 뻔했다. 0단계 검사 게이트가 「미번역을 어디서 한 번에 보는가」를 답할 때도
이 둘 중 어느 기준인지를 함께 정한다.
