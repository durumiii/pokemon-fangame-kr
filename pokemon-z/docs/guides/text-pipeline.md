...
**`translate/stage0/`가 ko의 파생으로 나란히 선다**(Z-53 이행 중 — 승격 전까지 정본은
여전히 ko다). ko를 직접 고쳤으면 **커밋한 뒤 `uv run translate/stage0/gen.py`로
재흡수하고 그것도 커밋한다** — 재흡수 없이 `stage0/emit.py --write`가 돌면 그 수정이
옛 값으로 되돌아간다(가드가 막지만, 규약으로 밟는 쪽이 먼저다). 쓰기가 stage0를 지나는
도구(지금은 `apply_verdicts`)는 이 걸음이 필요 없다.
