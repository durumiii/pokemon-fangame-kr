...
한 번 만진 것이 아니라 관리 단위의 선언이다 — 정본에서 몇 줄로 복사돼 있든 값은 한 판이고,
어느 자리를 고치든 전 자리가 함께 간다. 지키는 장치 셋:
- `verify.py`의 통일 검사 — 여러 맵 복제 원문의 값이 갈리면 FAIL(--strict). 의도된
  갈림(화자별 유지·손수정)만 `translate/data/divergence-allowed.jsonl`에 근거와 함께
  등재돼 있다. 갈림을 새로 허용하려면 이 원장에 판정을 등재하는 것이 유일한 길이다.
- 배치 재번역은 통일 원문을 **맵 경계 너머 한 번만** 번역한다(`batch_pages.dedupe`).
- 반영(`apply_verdicts`)은 값이 같던 전 자리에 새 판을 전파한다 — 현행이 다른 자리
  (의도된 갈림)는 값 불일치라 절대 안 퍼진다.
