...
겪는 순서가 없다. 자리 목록 줄에 `anchor`(`"m26.e4"`)와 `label`·`order`를 주면 그 이벤트의
대사 **뒤에** 「화면」으로 붙어, 플레이어가 말을 건 뒤 무엇을 어떤 차례로 만나는지가 한 장에
선다. 자리는 `k`(원문)나 `id`(사이트 id)로 가리키고 원문 하나에 자리가 여럿이면 `site`로
고른다. ⚠ 화면 자리의 판정 열쇠는 사이트 id 그대로라 `apply_verdicts`가 건너뛴다(수를
알린다) — 절마다 반영 경로가 달라 **사람이 읽고 그 절의 정본에 넣는다.**
