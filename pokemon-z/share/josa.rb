...
module JosaZ
  ...
  # ⚠ **바이트로 짓는다.** 거슬러 읽는 쪽은 `pack("C*")`로 만든 바이트열이라 딱지가
  # 없는데, 소스에 그냥 적은 「」는 루비 1.9+에서 UTF-8 딱지를 단다. 딱지가 다르면
  # 바이트가 같아도 `==`가 거짓이라, 최신 루비 실행기에서만 낫표·따옴표 건너뛰기가
  # 통째로 실패했다(1.8.7 9/9 · 3.1 8/9 — 2026-08-07 실물 루비 대조).
  SKIP_CHARS = [[0x29], [0x5D], [0x22], [0x27],
                [0xE2, 0x80, 0x99], [0xE2, 0x80, 0x9D],
                [0xE3, 0x80, 0x8D], [0xE3, 0x80, 0x8F]].map { |b| b.pack("C*") }
