...
class PokemonRegionMapScene
  ...
  # 칸은 (6,8)이다 — 원판은 표지가 (6,9) 근처에 어중간히 걸쳐 있었고, 그림 정본이
  # 그것을 한 칸 위로 올려 (6,8) 중앙에 앉혔다(유지자 판정 2026-08-21 — (6,9)로
  # 내리면 아래 칸의 마을 표지와 붙어 스냅이 지저분해진다). 옮김은
  # translate/assets/gen_regionmap.py의 MANUAL_MOVES에 적혀 있다 — 그 표와 이 칸이
  # 어긋나면 커서가 표지 아닌 칸에 선다.
  ...
  MISSING_POINTS = [
    [6,8,"Catacumbas Occidentales","서부 카타콤"]
  ]
